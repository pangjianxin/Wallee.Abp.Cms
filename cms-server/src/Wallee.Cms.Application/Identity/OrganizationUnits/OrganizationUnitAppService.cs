...
using Wallee.Cms.Identity.OrganizationUnits.Caches;
...
namespace Wallee.Cms.Identity.OrganizationUnits
{
    public class OrganizationUnitAppService : IdentityAppServiceBase, IOrganizationUnitAppService
    {
        ...
        public async virtual Task AddUsersAsync(Guid id, OrganizationUnitAddUserDto input)
        {
            ...
            {
                var userOrganizationUnits = user.OrganizationUnits;
                if (userOrganizationUnits.Any())
                {
                    foreach (var userOrganizationUnit in userOrganizationUnits)
                    {
                        await UserManager.RemoveFromOrganizationUnitAsync(userOrganizationUnit.UserId, userOrganizationUnit.OrganizationUnitId);
                    }
                }
        ...
        public async Task<ListResultDto<OrganizationUnitDto>> GetVisibleOrganizationUnitsAsync()
        {
            if (CurrentUser.IsInRole("admin"))
            {
                return new ListResultDto<OrganizationUnitDto>
                {
                    Items = (await OrganizationUnitCacheProvider.GetOrAddOrganizationUnitAsync()).Items
                };
            }
            return new ListResultDto<OrganizationUnitDto>
            {
        ...
        public async Task MoveUserAsync(OrganizationUnitMoveUserDto input)
        {
            ...
            await UserManager.RemoveFromOrganizationUnitAsync(input.IdentityUserId, input.OriginalOrganiationUnitId);

            await UserManager.AddToOrganizationUnitAsync(input.IdentityUserId, input.TargetOrganizationUnitId);
