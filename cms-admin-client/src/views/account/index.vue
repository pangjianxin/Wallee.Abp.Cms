                    ...
                    <v-form ref="passwordFormRef" v-model="passwordFormValid" @submit.prevent="submitChangePassword">
                        <v-card class="mt-5">
                            <v-card-text>
                                <v-row>
                                    <v-col cols="12" sm="6">
                                        <v-text-field label="当前密码" v-model="passwordForm.currentPassword" autocomplete="off"
...
let profileInfo: Ref<ProfileDto> = ref({});

onMounted(async () => {
    let res = await ProfileService.profileGet();
