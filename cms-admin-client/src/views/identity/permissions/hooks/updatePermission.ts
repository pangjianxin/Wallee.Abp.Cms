...
type PermissionGrantInfoViewModel = PermissionGrantInfoDto & {
  depth: number;
};

type PermissionGroupViewModel = Omit<PermissionGroupDto, "permissions"> & {
...
export const useUpdatePermissionForm = () => {
  ...
  const updateForm = reactive<UpdatePermissionsDto>({});
  ...
  const getPermissions = async (key: string, name: string) => {
    ...
    const groups = await PermissionsService.permissionsGet({
  ...
  //提交修改
  const submitUpdatePermission = async (e: SubmitEventPromise) => {
    updateFormValid.value = (await e).valid;
    if (!updateFormValid.value) return;
    ...
    try {
      ...
      updateForm.permissions = permissions.map((permission) => ({
