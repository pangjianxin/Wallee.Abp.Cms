import { IdentityRoleDto, RoleService } from "@/openapi";
import { DataTableHeader } from "typings/vuetify";

export const useIdentityRoleList = () => {
  const list = ref<IdentityRoleDto[]>([]);
  const filter = ref("");
  ...
  const getList = async () => {
    try {
      loading.value = true;
      const res = await RoleService.roleGetList({
        filter: filter.value,
